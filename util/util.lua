...
function util.getBatch(input, target, weight, idx, batchSize, trainSize) 
   local endidx = idx+batchSize-1
   if endidx > trainSize then
      endidx = trainSize
   end

   local x
   local y = target[{{idx,endidx}}]
   local w = weight[{{idx,endidx}}]
