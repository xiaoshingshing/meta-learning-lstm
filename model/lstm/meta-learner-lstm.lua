...
function getLearner(opt)
   ...
   local feval = function(x, inputs, targets, weights)
   ...
   learner.df = function(params, input, testY, weights)
      return feval(params, input, testY, weights)     
   end 
...
function getMetaLearner(opt)
   ...
   metaLearner.f = function(metaLearnerParams, learner, trainInput, trainTarget, 
      trainWeight, testInput, testTarget, steps, batchSize, evaluate)
...
--      print("start of learner training:",c1)
      -- training set loop
      for s=1,steps do 
...
--	    print(s,steps)
--	    print(i,trainSize)
            local x,y,w = util.getBatch(trainInput, trainTarget, trainWeight, i, batchSize, trainSize)

            -- get gradient and loss w/r/t learnerParams for input+label      
            local gradLearner, lossLearner = learner.df(learnerParams, x, y, w)                    
...
--      print("end of learner training:",c2)
...
collectgarbage("collect")
      ...
      collectgarbage("collect")
local c3 = collectgarbage("count")
--      print("after autograd:",c3)
  
   metaLearner.dfWithGradNorm = function(metaLearnerParams, learner, trainInput, 
      trainTarget, trainWeight, testInput, testTarget, layers, prevState, steps)  
   
      local grads, loss, pred = metaLearner.df(metaLearnerParams, learner, 
         trainInput, trainTarget, trainWeight, testInput, testTarget, layers, prevState, 
         steps)  
      collectgarbage("collect")
      collectgarbage("collect")
      collectgarbage("collect")
